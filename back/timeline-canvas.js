...
var canvansW = $('#timeline').parent().width();
...
function mouseupFunc(e){
    ...
    if(g_isMousemove){ //拖动 事件
		...
		window.stat = 0;
		console.log((left*(zoom * 3600 * 1000) / canvansW))
...
$("#setTime").click(function(){
	var time = 0;
	var year = $('#dataYearID').val();
	var hour = parseInt($('#dataHoursID').val());
	var min =  parseInt($('#dataMinID').val());
	var sec =  parseInt($('#dataSecID').val());
		hour = hour<10?'0'+hour:hour;
		min = min<10?'0'+min:min;
		sec = sec<10?'0'+sec:sec;
		var time = new Date(year+' '+hour+':'+min+':'+sec).getTime()
		set_time_to_middle(time);
		initIntetval(true,time);
})
/* 刻度放大 */
$('#addCalibration').click(function(e){
	initMouseWheelfucn(e,1)
})
/* 刻度缩小 */
$('#subCalibration').click(function(e){
	initMouseWheelfucn(e,-1)
})
