	...
	jRange.prototype = {
		...
		template: '<div class="slider-container">\
			<div class="back-bar">\
                ...
                <div class="pointer high"></div><div class="pointer-label bafont">456789</div>\
		...
		onDrag: function(pointer, e) {
			...
			//修改内容
			this.changeLabel(position);
			
		},
		changeLabel:function(position,animate){
			var fa = $(this.clickableBar).width();
		/* 	if(position<70){
				position = 0;
			} */
			...
			this.highLabel[animate ? 'animate' : 'css']({
		...
		setPosition: function(pointer, position, isPx, animate) {
			...
			if (this.isSingle()) {
				...
				this.changeLabel(position,animate);
		...
		renderScale: function() {
			...
			for (var i = 0; i < s.length; i++) {
				str += '<span style="left: ' + i * prc + '%">' + (s[i] != '|' ? '<ins>' + s[i] + '</ins>' : '') + '<span>1</span></span>';
			}
			this.scale.html(str);
		...
		formatFunc:function(value){
			...
			//value = value*60
			var times = this.setTimeHMS(value)
		/* 	if(hours<10){
				hours = '0'+hours;
			}
			if(mins<10){
